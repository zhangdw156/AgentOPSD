...
class FSDPVLLMShardingManager(BaseShardingManager):
    ...
    def _format_cleanup_failure(cleanup_error, action):
        try:
            return (
                f"FSDP-vLLM cleanup failed during {action}: "
                f"{cleanup_error!r}"
            )
    ...
    def _record_cleanup_failure(
        ...
        note = self._format_cleanup_failure(
            cleanup_error,
            action,
        )
        self._tainted = True
    ...
    def _rng_states_equal(current_state, expected_state):
        if (
            ...
            and isinstance(expected_state, torch.Tensor)
        ):
    ...
    def _cleanup_enter_state(self, original_error=None):
        ...
        if (
            self._enter_rng_switch_attempted
    ...
    def __enter__(self):
        if self._tainted:
            raise RuntimeError(
                "FSDP-vLLM sharding manager is tainted: "
                f"{self._taint_reason}"
            )
        if (
            ...
            raise RuntimeError(
                "FSDP-vLLM sharding manager is already entered"
            )
    ...
    @GPUMemoryLogger(
        role="fsdp vllm sharding_manager",
        logger=logger,
    )
