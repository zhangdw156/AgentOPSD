...
def validate_trajectory_grpo_config(
    ...
    *,
    ...
    scheduler = trajectory_config.get(
        "scheduler",
        NATIVE_TRAJECTORY_GRPO_CONFIG["scheduler"],
    )
    reducer = trajectory_config.get(
        "reducer",
        NATIVE_TRAJECTORY_GRPO_CONFIG["reducer"],
    )
    advantage = trajectory_config.get(
        "advantage",
        NATIVE_TRAJECTORY_GRPO_CONFIG["advantage"],
    )
    penalty = trajectory_config.get(
        "penalty",
        NATIVE_TRAJECTORY_GRPO_CONFIG["penalty"],
    )
    filter_mode = trajectory_config.get(
        "filter",
        NATIVE_TRAJECTORY_GRPO_CONFIG["filter"],
    )
    allowed_values = {
    ...
    if reducer not in {"token_mean", "trajectory_mean"}:
        raise ValueError(
            f"unsupported trajectory_grpo.reducer: {reducer}"
        )
    for name, value in values.items():
        if value not in allowed_values[name]:
            raise ValueError(
                f"unsupported trajectory_grpo.{name}: {value}"
            )
    ...
    if (
        ...
        raise ValueError(
            "penalty-aware filter is computed during rollout and is "
            "incompatible with algorithm.use_kl_in_reward=True"
        )
    if scheduler not in {"trajectory", "trajectory_packed"}:
        return

    actor = (
        config.get("actor_rollout_ref", {}).get("actor", {})
        if "algorithm" in config
        else {}
    )
    ...
    if ulysses_sequence_parallel_size is None:
        ulysses_sequence_parallel_size = actor.get(
            "ulysses_sequence_parallel_size"
        )
    if actor_strategy is None or not str(actor_strategy).lower().startswith(
        "fsdp"
    ):
        raise ValueError(
            "phase-I trajectory schedulers require an FSDP actor strategy"
        )
    if ppo_epochs != 1:
        raise ValueError(
            "phase-I trajectory schedulers require ppo_epochs=1"
        )
    if ulysses_sequence_parallel_size != 1:
        raise ValueError(
            "phase-I trajectory schedulers require "
...
def assign_trajectory_update_ids(
    ...
    *,
    ...
    if real_row_count == 0:
        raise ValueError(
            "trajectory packing requires at least one real row"
        )
    ...
    if update_count > len(grouped):
        raise ValueError(
            "trajectory packing cannot preserve the native optimizer "
            f"cadence: {update_count} updates requested for "
            f"{len(grouped)} trajectories"
        )
    ...
    for trajectory_id, row_indices in grouped.items():
        update_ids[np.asarray(row_indices, dtype=np.int64)] = (
            trajectory_to_update[trajectory_id]
        )
...
def group_rows_by_uid_traj_uid(
    ...
    for row_index, key in enumerate(
        zip(uid_array.tolist(), traj_uid_array.tolist())
    ):
...
def count_trajectory_invalids(
    ...
    *,
    ...
    if sum(len(group.row_indices) for group in groups) != len(
        invalid_counts
    ):
        raise ValueError(
            "row_invalid_counts must match uids and traj_uids"
        )
    weights = (
    ...
    _require_same_length(
        row_invalid_counts=invalid_counts,
        row_weights=weights,
    )
    return np.asarray(
        [
...
def reduce_trajectory_scores(
    ...
    *,
    ...
    if reducer != "mean_row":
        raise ValueError(
            f"unsupported trajectory score reducer: {reducer}"
        )
    ...
    for group in groups:
        ...
        if denominator <= 0:
            raise ValueError(
                f"trajectory {group.traj_uid!r} has no real rows"
            )
        reduced.append(
            (scores[group_indices] * group_weights).sum() / denominator
        )
...
def select_penalty_aware_group_indices(
    ...
    *,
    ...
    processed_rewards = (
        rewards
        - float(invalid_action_penalty_coef) * invalid_counts
    )
    grouped = {}
    ...
    for indices in grouped.values():
        ...
        if np.ptp(processed_rewards[group_indices]) > 0:
...
def require_filter_target_reached(
    ...
    if collected_trajectories < target_trajectories:
        raise RuntimeError(
            ...
            f"{collected_trajectories}/{target_trajectories} "
...
def take_complete_uid_groups(
    ...
    if max_trajectories < 0:
        raise ValueError(
            "max_trajectories must be non-negative"
        )
    uid_array = _as_1d("uids", uids, dtype=object)
    grouped = {}
    for index, uid in enumerate(uid_array.tolist()):
    ...
    *,
    ...
    for indices in grouped.values():
        ...
        if normalize_by_std and len(group_indices) > 1:
            centered = centered / (
                group_scores.std(ddof=1) + epsilon
            )
...
def broadcast_trajectory_values(
    ...
    if len(groups) != len(values):
        raise ValueError(
            f"expected {len(groups)} trajectory values, got {len(values)}"
        )
    row_values = np.empty(
...
def make_zero_weight_padding(
    ...
    if target_size < num_real:
        raise ValueError(
            f"target_size={target_size} is smaller than {num_real} real rows"
        )
    if target_size > 0 and num_real == 0:
        raise ValueError(
            "cannot pad an empty update to a non-zero target size"
        )
    num_padding = target_size - num_real
    if num_padding:
        padded_indices = np.concatenate(
            (indices, np.resize(indices, num_padding))
        )
    else:
        padded_indices = indices.copy()
    weights = np.concatenate(
        (
            np.ones(num_real, dtype=np.float64),
            np.zeros(num_padding, dtype=np.float64),
        )
    )
